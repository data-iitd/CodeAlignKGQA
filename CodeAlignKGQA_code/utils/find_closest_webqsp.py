...
f = open("../../dataset/webQSP/raw_kb.txt", "r")
kb_file = f.readlines()
f.close()

entity_name_to_ids, kb = processKB(kb_file)

# f = open("../../dataset/webQSP/webqsp/" + sys.argv[2])
# data = json.load(f)
# f.close()
...
def nearest_kv(predicate, type_, list_relation, idx):
    ...
    print("topK:", top_k)
    return top_k[0]
    # return select_entitiy(top_k, data['Questions'][idx]['RawQuestion'], predicate)
...
def nearest_neigh(word, type_, idx):
    ...
    print("topK: ", top_k)
    return top_k[0]
    # return select_entitiy(top_k, data['Questions'][idx]['RawQuestion'], word)
