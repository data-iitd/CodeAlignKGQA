...
kb = json.load(open(os.path.join('./preprocessed_kb/', 'kb.json')))

# f = open("../../dataset/KQAPro.IID/val.json")
# data = json.load(f)
# f.close()
...
def convert_string_to_triple(temp):
    flag = False
    start = 0
    out = []
    count = 0
...
def select_entitiy_new(context, question, word, type_):
    prompt = f"From below triples list, select the {type_} that belong to the given question. Triples list = [" + ', '.join(context)  + "]"
    sentence = question + f" {type_} extracted from Question - "  + word
    options = context
...
#                 {{prompt}}
#                 Question : {{que}}
#                 Output : {{~gen "answer" n=1 max_tokens = 200 stop = "\n"}}
#                 ''')
...
#                             )
    ...
    ans = convert_string_to_triple(ans)
    print('selected_triple ', ans)
    if(type_ == "entity" or type_ == "concept"):
        ans = ans[0]
    elif(type_ == "relation"):
        ans = ans[1]
    else:
...
def nearest_kv(predicate, type_, list_relation, idx):
    ...
    print("topK: ", top_k)
    return top_k[0]
    # return select_entitiy_new(rag_contexts[idx], data[idx]['question'], predicate, type_)
...
def nearest_neigh(word, type_, idx):
    ...
    if(type_ == 'e'):
        ...
        for key, value in kb['concepts'].items():
            nbas.append(value['name'])
    nbas = list(set(nbas))
    # print("input_list: ", nbas)
    if(word in nbas):
        print('ans ', word)
        return word
    ...
    print("topK: ", top_k)
    return top_k[0]
    # return select_entitiy_new(rag_contexts[idx], data[idx]['question'], word, type_)
